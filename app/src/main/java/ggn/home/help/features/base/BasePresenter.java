...
//        views are created ,now its time to initialize them..
    ...
    protected final <G> G getRetrofitInstance(Class<G> retroInterface) {
        return ApplicationClass.getRetrofit(getView().getActivityG()).create(retroInterface);
    }


    /**
     * Create a network call and return back the response using {@link retrofit2.Callback}
                    ...
                    public void onError(@io.reactivex.annotations.NonNull Throwable e) {
                        ...
                        try {
                            ...
                            if (string == null || string.isEmpty()) {
                                ...
                                getView().displayError(new JSONObject(string).optString("error"));
                                onErrorInAPI(string);
                            }
